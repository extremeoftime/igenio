...
def registration():
    ...
    registr_login = Entry()
    text_password1 = Label(text="Введите вашпароль:")
...
def login():
    ...
    enter_login = Entry()
    text_enter_password = Label(text="Введите вашпароль:")
    ...
    def log_pass():
        ...
        if enter_login.get() in a:
            if enter_password.get() == a[enter_login.get()]:
                messagebox.showinfo("ход выполнен","Привет! У тебя 5 новых сообщений!")
            else:
                messagebox.showerror("Ошибка!","Вы ввели неверный логин или пароль")
        else:
            messagebox.showerror("Ошибка!","ы ввели неправильный логин!")
